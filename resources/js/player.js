document.addEventListener("DOMContentLoaded", function () {
    ...
    const fullscreenPlayerEmbed = document.getElementById(
        "fullscreenPlayerEmbed"
    );
    const playButton = document.getElementById("playButton");
    const fullscreenPlayButton = document.getElementById(
        "fullscreenPlayButton"
    );
    const closePlayerButton = document.getElementById("closePlayerButton");
    const fullscreenPlayerButton = document.getElementById(
        "fullscreenPlayerButton"
    );
    const closeFullscreenPlayer = document.getElementById(
        "closeFullscreenPlayer"
    );
    const closeFullscreenPlayer2 = document.getElementById(
        "closeFullscreenPlayer2"
    );
    const progressBar = document.getElementById("progressBar");
    const fullscreenProgressBar = document.getElementById(
        "fullscreenProgressBar"
    );
    const fullscreenProgressHandle = document.getElementById(
        "fullscreenProgressHandle"
    );
    const fullscreenProgressContainer = document.getElementById(
    ...
    function showDetailSong(songId) {
        if (isPlayerVisible) {
            fullscreenPlayer.classList.remove("hidden");
            fullscreenPlayer.setAttribute("data-id", songId);
            document.body.style.overflow = "hidden"; // Prevent background scrolling
        }
    ...
    function fetchSongDetail(songId) {
        fetch(`/show-song/${songId}`)
            ...
            .then((data) => {
                console.log("Song Data:", data); // Do something with the data
                const embedUrl = data.data.embedUrl;
                const coverUrl = data.data.embedUrl;
                const lyrics = data.data.song.lyrics;
                const embedIframe = document.getElementById(
                    "fullscreenPlayerEmbed"
                );
                const elLyrics = document.getElementById(
                    "fullscreenPlayerLyrics"
                );
                const elSongTitle = document.getElementById(
                    "fullscreenPlayerSongTitle"
                );
                const elArtist = document.getElementById(
                    "fullscreenPlayerArtist"
                );
                const elCoverAvatar = document.getElementById(
                    "fullscreenPlayerCoverAvatar"
                );
                const elAlbum = document.getElementById(
                    "fullscreenPlayerAlbum"
                );

                if (embedIframe && embedUrl) {
                    embedIframe.src = embedUrl;
                } else {
                    embedIframe.src = "";
                }
                if (elLyrics && lyrics) {
                    elLyrics.innerHTML = lyrics.replace(/\n/g, "<br>");
                }
                elSongTitle.innerHTML = data.data.song.title;
                elArtist.innerHTML = data.data.artistName;
                if (data.data.album && data.data.album.title) {
                    elAlbum.innerHTML = `<div class="text-gray-300 text-sm">${data.data.album.title}</div>`;
                } else {
                    elAlbum.innerHTML = ``;
                }
            })
    ...
    if (moreOptionsButton && playerOptionsMenu) {
        ...
        document.addEventListener("click", function (e) {
            if (
                ...
                !playerOptionsMenu.contains(e.target)
            ) {
    ...
    const fullscreenLikeButton = document.getElementById(
    ...
    if (likeButton) {
        likeButton.addEventListener("click", function () {
            ...
            if (fullscreenLikeButton) {
                ...
                fullscreenLikeButton.innerHTML = this.innerHTML.replace(
                    '"></i>',
                    ' text-xl"></i>'
                );
            }
    ...
    if (fullscreenProgressContainer) {
        fullscreenProgressContainer.addEventListener("click", function (e) {
            ...
            currentSong.currentTime =
                (progressPercentage / 100) * currentSong.duration;
    ...
    document.querySelectorAll(".play-song-btn").forEach((button) => {
        button.addEventListener("click", function (e) {
            ...
            if (!this.hasAttribute("data-login-required")) {
                ...
                const artist =
                    this.getAttribute("data-artist") || "Unknown Artist";
                const cover =
                    this.getAttribute("data-cover") ||
                    "https://via.placeholder.com/300";
                const album =
                    this.getAttribute("data-album") || "Unknown Album";
                const songId =
                    this.getAttribute("data-id") || Date.now().toString();

                const duration = parseInt(
                    this.getAttribute("data-duration") || 300
                );
    ...
    const fullscreenVolumeSlider = document.getElementById(
        "fullscreenVolumeSlider"
    );
    const muteButton = document.getElementById("muteButton");
    const fullscreenMuteButton = document.getElementById(
    ...
    const fullscreenShuffleButton = document.getElementById(
    ...
    const fullscreenRepeatButton = document.getElementById(
    ...
    function updateRepeatButtonUI(mode) {
        ...
        if (fullscreenRepeatButton) {
            if (mode === "none") {
                fullscreenRepeatButton.innerHTML =
                ...
                fullscreenRepeatButton.innerHTML =
                    '<i class="ti ti-repeat-once text-xl"></i>';
                fullscreenRepeatButton.classList.add("text-red-500");
            }
        }
    }
    ...
    const fullscreenPrevButton = document.getElementById(
    ...
    const fullscreenNextButton = document.getElementById(
            ...
            if (typeof Swal !== "undefined") {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Clear queue items
                        const queueItems =
                            document.getElementById("queueItems");
                        if (queueItems) {
    ...
    document.addEventListener("keydown", function (e) {
        ...
        if (
            e.code === "Escape" &&
            !fullscreenPlayer.classList.contains("hidden")
        ) {
            fullscreenPlayer.classList.add("closing");
            setTimeout(() => {
                fullscreenPlayer.classList.remove("closing");
        ...
        if (playerBar) {
            ...
            playerBar.addEventListener("click", function (e) {
                ...
                if (window.innerWidth <= 768) {
                    ...
                    if (
                        !e.target.closest(".player-button") &&
